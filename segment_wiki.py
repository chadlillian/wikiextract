    ...
    try:
        ...
        for idx, article in enumerate(article_stream):
            ...
            if include_interlinks:
                ...
                output_data["coordinates"] = coords
...
def segment(page_xml, include_interlinks=False):
    ...
    categories_regex = r"\[\[Category:.*[^\[\]\v]\]\]"
    coords_regex =  r"\{\{Coord.*[^\{\}\v]\}\}"

    title = elem.find(title_path)
    ...
    if text is not None:
        ...
        coords = re.findall(coords_regex,text)
    ...
    categories = [cc.split(':',1)[1].strip(' ]|*') for cc in categories]

    if include_interlinks:
        return title, sections, interlinks,categories, coords
    else:
        ...
        for group in utils.chunkize(page_xmls, chunksize=10 * self.processes, maxsize=1):
            for article in pool.imap(partial(segment, include_interlinks=self.include_interlinks),
                ...
                if not sections or sections[0][1].lstrip().lower().startswith("#redirect"):  # filter redirect
                    try:
                        print('XXXX ',article_title,' ::: ',sections[0][1].strip())
                    except:
                        None
                    skipped_redirect += 1
                    continue
                ...
                if self.include_interlinks:
                    ...
                    categories = article[3]
                    coords = article[4]
                    yield (article_title, sections, interlinks,categories,coords)
                else:
